describe('Покупка нового аватара', function () {
    
    it('Проверка покупки нового аватара для моего тренера', function () {
        cy.visit('https://pokemonbattle.me/');
        cy.get(':nth-child(1) > .auth__input').type('annushka.chursina@yandex.ru');
        cy.get('#password').type('Molly65536');
        cy.get('.auth__button').click();
        cy.get('.header__btns > [href="/shop"]').click();
        cy.get('.pokemon__title').contains('Магазин');
        cy.get('.shop__list > li').not('.feature-empty').children('.shop__button').eq(0).click();
                cy.get('.pay__main-header').contains('Пикачунькофф');
        cy.get('.pay__payform-v2 > :nth-child(2) > .pay_base-input-v2').type('4620869113632996');
        cy.get(':nth-child(1) > .pay_base-input-v2').type('1225');
        cy.get('.pay-inputs-box > :nth-child(2) > .pay_base-input-v2').type('125');
        cy.get('.pay__input-box-last-of > .pay_base-input-v2').type('ANNA CHURSINA');
        cy.get('.pay__main-v2').click();
        cy.get('.pay-btn').click();
        cy.get('.payment__fielheader').contains('Подтверждение покупки');
        cy.get('.payment__label').contains('Код из пуша или СМС');
        cy.get('.payment__submit-button').should('be.disabled');
        cy.get('#cardnumber').type('56456');
        cy.get('.payment__submit-button').should('be.enabled');
        cy.get('.payment__submit-button').click();
        cy.get('.payment__font-for-success').contains('Покупка прошла успешно');
        cy.get('.payment__adv').click();

		})

})

